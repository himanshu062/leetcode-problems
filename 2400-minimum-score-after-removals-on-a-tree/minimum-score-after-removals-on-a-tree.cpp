        ...
        while (!q.empty()) {
            ...
            for (int next : graph[cur]) {
                if (!seen[next]) {
                    ...
                    children[next].insert(children[cur].begin(),
                                          children[cur].end());
        ...
        for (int i = 0; i < m - 1; ++i) {
            for (int j = i + 1; j < m; ++j) {
                ...
                if (children[a].count(c)) {
                    vals = {xor_val[c], xor_val[a] ^ xor_val[c],
                            total ^ xor_val[a]};
                } else if (children[c].count(a)) {
                    vals = {xor_val[a], xor_val[c] ^ xor_val[a],
                            total ^ xor_val[c]};
                } else {
                    vals = {xor_val[a], xor_val[c],
                            total ^ xor_val[a] ^ xor_val[c]};
                }
